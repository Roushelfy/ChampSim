...
struct spp_gsp_tiered : public champsim::modules::prefetcher {
  static constexpr const char* VARIANT_TAG = "BWC_GSP_TIERED";
  ...
  // No toggle flag: spp_gsp_tiered is always the BWC version.
  ...
  // Keep the local hard-congestion path as a safety net; OpenEvolve only tunes the
  // global shared-pressure controller layered on top.
  ...
  static constexpr double BWC_RELIEF_ACC      = 0.05;

  enum class pressure_mode_t {
    avg,
    max,
    avg_or_max,
  };
  ...
  struct controller_params {
    ...
    double global_llc_t1         = 0.30;
    double global_llc_t2         = 0.38;
    double global_llc_t3         = 0.44;
    uint32_t tier1_issue_period  = 2;
    uint32_t tier2_issue_period  = 4;
    uint32_t tier3_pf_threshold  = 60;
    uint32_t min_active_cores    = 2;
    uint32_t congested_epochs    = 2;
  ...
  void load_runtime_params();
  void record_pressure_sample();
  void finalize_pressure_epoch();
  [[nodiscard]] bool pressure_at_or_above(double avg_value, double max_value, double threshold) const;
  [[nodiscard]] uint32_t select_global_tier(double avg_mshr_util, double max_mshr_util, double avg_llc_rq_util, double max_llc_rq_util) const;
