...
All experiments were conducted on the **ChampSim** cycle-accurate microarchitectural simulator. Each simulation consisted of a **10M-instruction warmup phase** followed by a **50M-instruction simulation phase**. Memory subsystem parameters match a realistic DDR4-3200 system with a single DRAM channel, a private per-core L2C (1 MB, 8-way), and a shared LLC (4 MB, 16-way).
...
|----------|---------|-----|----------|---------|---------|
| **`mcf` IPC** | 0.1163 | **0.1255** (+8.0%) | 0.1212 | 0.1165 (−3.9%) | 0.1164 (−4.0%) |
...
**SPP dominates BOP on `lbm`.** For the streaming stencil workload, SPP\_Orig achieves 0.8482 IPC versus BOP's 0.7995 (−5.8%). BOP's single offset=9 captures lbm's array stride partially but issues only 324K prefetches. SPP's path-confidence mechanism identifies the stride signature and systematically generates 3.2M prefetches—approximately 10× more coverage—while maintaining similar DRAM bus efficiency (AVG 7.524 vs. BOP's 7.480 cycles). For structured streaming workloads, the breadth of path-based prediction outweighs BOP's simplicity advantage.
...
The result is **asymmetric throttling**: the polluter is suppressed; the victim is protected. lbm achieves **0.7933 IPC**, virtually identical to its uncontended single-core performance (0.8482 × 0.935 = 0.7931). BWC's Weighted Speedup of **1.253** is **5.1% above FDP's 1.192**.

### 3.4 Insight V: The Self-Contention Twist — Why FDP Accidentally Helps `mcf`

The most counterintuitive result in the multi-core experiment is that FDP raises `mcf`'s IPC by **49%** (0.0395 → 0.0589). This is not a benefit of better prefetching for `mcf`; it is the result of FDP eliminating `mcf`'s *self-inflicted bandwidth contention*.

FDP's Level 1 increases `pf_threshold` from 25 to **80**, acting as a **quality filter**: only SPP patterns with high confidence (>80%) are allowed to generate prefetch requests. For `mcf`, whose access patterns produce almost exclusively low-confidence SPP signatures, this eliminates nearly all prefetch traffic. The LLC read queue and DRAM, now freed from `mcf`'s garbage prefetches, can service `mcf`'s *demand* accesses much more quickly. `mcf` thus completes its 50M instructions in 849M cycles rather than 1.265B, a substantial improvement.

BWC's rate-limiter (`issue_period=4`) achieves a similar **75% reduction** in prefetch volume but does *not* produce the same IPC improvement for `mcf`. The rate-limiter is a **quantity filter**, not a quality filter: it emits every fourth prefetch candidate regardless of confidence. The surviving prefetches have the same confidence distribution as the original stream, including low-confidence entries that occupy DRAM bandwidth without providing value to `mcf`'s demand requests. The distinction—quality filtering versus quantity thinning—explains why identical prefetch count reductions produce different outcomes for the polling core.

This result reveals an orthogonal design dimension: the *type* of throttling actuator matters independently of the throttling *decision*. A future iteration of BWC could adopt a confidence-gated actuator (raising `pf_threshold`) rather than a rate-limiter to obtain both the bandwidth reduction and the self-contention elimination benefit.
...
6. **Actuator design is an orthogonal dimension.** FDP's quality-filter actuator (`pf_threshold` elevation) eliminates self-contention within a core's own prefetch stream, producing a 49% IPC improvement for `mcf`. BWC's rate-limiter actuator does not achieve this effect. Combining BWC's superior *sensing* with FDP's quality-filter *actuation* is a promising direction for a next-generation controller.
